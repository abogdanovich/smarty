...
%endif
%if %{build_perl}
%define perl_archlib %(eval "`/usr/bin/perl -V:installarchlib`"; echo $installarchlib)
%define perl_sitearch %(eval "`/usr/bin/perl -V:installsitearch`"; echo $installsitearch)
%define perl_sitelib %(eval "`/usr/bin/perl -V:installsitelib`"; echo $installsitelib)
%define perl_siteman3dir %(eval "`/usr/bin/perl -V:siteman3dir`"; echo $siteman3dir)
rm -f $RPM_BUILD_ROOT%{perl_archlib}/perllocal.pod
rm -f $RPM_BUILD_ROOT%{perl_sitearch}/auto/OW/.packlist
rm -f $RPM_BUILD_ROOT%{perl_sitearch}/auto/OWNet/.packlist
%endif
%if %{build_python}
%define python_sitedir %(python -c "from distutils.sysconfig import get_python_lib; print get_python_lib(plat_specific=1)" 2>/dev/null|| echo /usr/lib/python2.7/dist-packages)
...
%define php_extdir %(php-config --extension-dir 2>/dev/null || php5-config --extension-dir 2>/dev/null || php4-config --extension-dir 2>/dev/null || echo )
#don't use the PHPLIBDIR here... assume php-config works instead.
